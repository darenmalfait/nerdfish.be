...
There are two options to do this. You can find the first way on [the respective wiki page](https://daren.be/wiki/raspberry-pi-activate-ssh-and-wifi-headless). We will handle the second way when we come to it.
...
* static ip\_address: Fill in the IP address you would like, followed by a \`/\` en the subnet mask of your network. If you use 255.255.255.0, then you fill in \`/24\`. If you have a broader subnet mask like 255.255.0.0, you can use /16.
* static routers: Fill in the router's IP address (default gateway).
* static domain\_name\_servers: Here, you can fill in which DNS servers you want to use.
