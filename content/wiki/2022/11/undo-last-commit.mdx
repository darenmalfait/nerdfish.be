...
git reset --soft HEAD~1 Will move the HEAD pointer to the previous commit will
leave the changes in the working directory and the index intact. This means that
...
git reset --hard will undo the commit and discarding all the changes.
